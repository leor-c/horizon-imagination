"""
Code Taken from 
https://github.com/nvidia-cosmos/cosmos-predict2/blob/main/cosmos_predict2/models/text2image_dit.py

Modified and simplified for the needs of this project.
"""
...
from horizon_imagination.models.world_model.rope import apply_rotary_pos_emb
...
def torch_attention_op(
    ...
    Args:
        ...
        attn_mask: a mask to be applied with shape (1, 1, q_seq_len, kv_seq_len)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if backend not in ["torch"]:  # transformer_engine
        ...
        def get_normalization_layer():
            if backend == 'transformer_engine':
                # return te.pytorch.RMSNorm(self.head_dim, eps=eps)
                raise NotImplementedError(f'No support for backend="{backend}"')
            elif backend == 'torch':
                return nn.LayerNorm(self.head_dim, eps=eps)
            else:
                raise NotImplementedError(f'No support for backend="{backend}"')
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.x_dim = x_dim
        self.layer_norm_self_attn = nn.LayerNorm(x_dim, elementwise_affine=False, eps=eps)
        ...
        self.layer_norm_mlp = nn.LayerNorm(x_dim, elementwise_affine=False, eps=eps)
...
class MiniTrainDIT(nn.Module):
    ...
    def __init__(
        ...
        self.c_embedder = TimestepEmbedding(model_channels, model_channels, use_adaln_lora=use_adaln_lora)
        ...
        # self.t_embedding_norm = te.pytorch.RMSNorm(model_channels, eps=ln_eps)
        self.t_embedding_norm = nn.RMSNorm(model_channels, eps=ln_eps)
        self.init_weights()
    ...
    def unpatchify(self, x_B_T_H_W_M: torch.Tensor) -> torch.Tensor:
        x_B_Tt_C_Hp_Wp = rearrange(
            x_B_T_H_W_M,
            "B T H W (p1 p2 t C) -> B (T t) C (H p1) (W p2)",
            p1=self.patch_spatial,
            p2=self.patch_spatial,
            t=self.patch_temporal,
        )
        return x_B_Tt_C_Hp_Wp
    ...
    def forward(
        ...
        if condition_emb is not None:
            ...
            c_B_T_D = t_embedding_B_T_D + condition_emb
...
class DiT(nn.Module, Configurable):
    ...
    def __init__(self, config: Config):
        super().__init__()
        self.model = MiniTrainDIT(
            max_img_h=config.max_img_h,
            max_img_w=config.max_img_w,
            max_frames=config.expected_max_frames,
            ...
            atten_backend=config.atten_backend,
            use_adaln_lora=config.use_adaln_lora,
            adaln_lora_dim=config.adaln_lora_dim,
            rope_h_extrapolation_ratio=config.rope_h_extrapolation_ratio,
            rope_w_extrapolation_ratio=config.rope_w_extrapolation_ratio,
            rope_t_extrapolation_ratio=config.rope_t_extrapolation_ratio,
